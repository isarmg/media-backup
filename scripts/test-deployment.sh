...
[[ -n "$archive_arg" ]] ||
  fail "usage: test-deployment.sh /absolute/path/photo-backup-server-0.2.0-x86_64-unknown-linux-gnu.tar.gz"
...
assert identity["product"] == "photo-backup-server"
...
assert identity["release_contract_sha256"] == "3a65b1a129118beeafe552c42d27812320d08bdde7966cedc9ac1e5476e995e9"
PY
...
[[ "$verification" == PHOTO_BACKUP_RELEASE_VERIFIED_V1$'\tphoto-backup-server\t0.2.0\t'"$source_revision"$'\tx86_64-unknown-linux-gnu\t'"$contract" ]] ||
...
cmp --silent "$release_root/share/web/admin.html" "$project_dir/crates/server/src/admin.html" ||
  fail "archive omits or changes the real admin HTML"
cmp --silent "$release_root/share/web/admin.css" "$project_dir/crates/server/src/admin.css" ||
...
expect_invalid_source() {
  ...
  if PHOTO_BACKUP_SETUP_ROOT="$rejected_root" PHOTO_BACKUP_SETUP_TEST=1 \
...
fresh_negative
printf '#!/usr/bin/env bash\nexit 0\n' >"$negative_root/bin/photo-backup-server"
chmod 0755 "$negative_root/bin/photo-backup-server"
...
fresh_negative
chmod 4755 "$negative_root/bin/photo-backup-server"
...
import json, pathlib, sys
path = pathlib.Path(sys.argv[1]); value = json.loads(path.read_text())
value["identity"][sys.argv[2]] = "not-photo-backup"
path.write_text(json.dumps(value) + "\n")
PY
...
value["identity"].update({
    ...
    "mobile_ffi_epoch": "photo-backup-mobile-v0.1-r1",
})
path.write_text(json.dumps(value) + "\n")
PY
...
[[ -d "$install_root/var/lib/isarmg/photo-backup/db" &&
  -d "$install_root/var/lib/isarmg/photo-backup/data" ]] || fail "separate database/data directories are missing"
...
PHOTO_BACKUP_SETUP_ROOT="$conflict_root" PHOTO_BACKUP_SETUP_TEST=1 "$setup_script" >/dev/null
conflict_file="$conflict_root/opt/isarmg/photo-backup/releases/$version/README.md"
printf '\nconflict\n' >>"$conflict_file"
conflict_digest="$(sha256sum "$conflict_file" | awk '{print $1}')"
if PHOTO_BACKUP_SETUP_ROOT="$conflict_root" PHOTO_BACKUP_SETUP_TEST=1 \
  "$setup_script" >/dev/null 2>&1; then
...
mkdir -p "$old_current_root/opt/isarmg/photo-backup/releases/0.1.0"
ln -s releases/0.1.0 "$old_current_root/opt/isarmg/photo-backup/current"
if PHOTO_BACKUP_SETUP_ROOT="$old_current_root" PHOTO_BACKUP_SETUP_TEST=1 \
  "$setup_script" >/dev/null 2>&1; then
...
touch "$hardlink_config_root/etc/isarmg/photo-backup.env"
ln "$hardlink_config_root/etc/isarmg/photo-backup.env" "$hardlink_config_root/config-alias"
if PHOTO_BACKUP_SETUP_ROOT="$hardlink_config_root" PHOTO_BACKUP_SETUP_TEST=1 \
  "$setup_script" >/dev/null 2>&1; then
